...
⚠️ 版本化 Delete 标记全部同 code 版本为 deprecated（应只标记指定版本）

⚠️ `_doList` 版本过滤用 `int8(1)` vs MongoDB bool `true` 类型不匹配

⚠️ ReferenceRelation 不支持点分路径 Field（如 `fields.dept_id`），需在 expandGet 前把 FK 值提到顶层

⚠️ `CRUDRepo()` 对 MongoDB 返回 nil，`_doDelete` 版本化路径也有同样风险
...
- [ ] ReferenceRelation 支持点分路径 Field
- [ ] 版本化 Delete 只标记指定版本
- [ ] MongoDB `isCurrent` bool vs int8 类型匹配
- [ ] `_doDelete` 版本化路径 MongoDB 适配（同 _doUpdate 的 CRUDRepo nil 问题）
