...
type CascadeRelation struct {
	...
	// MergeChildrenOnUpdate 更新父记录**且请求携带子表**时，是否把请求子记录与旧子
	// 记录按身份**逐字段合并**（而不是把请求当作子记录的完整内容直接替换/重建）。
	//
	// 默认 false = 既有契约（两条父路径各自保持原语义）：
	//   - 版本化父  ：请求子数组即新版本的完整子内容，字段缺省 → 零值/默认值；
	//   - 非版本化父：先删旧子行，再按请求子数组「全量替换」。
	// 置 true 后，**两条路径都**先按身份配对、以旧记录为基底合并，再走各自既有语义
	// （版本化父重建新子快照；非版本化父删旧行后用合并结果替换），因此「携带子表
	// 局部更新」不会再把未提交字段归零。
	//
	// 语义（与「提交哪些字段就改哪些字段」的契约配套）：
	//
	//	① 按身份把请求子记录与旧子记录配对（旧主键 → PublishCodeField 业务 code 兜底）；
	//	② 配对成功 → 以旧记录为基底，用请求字段逐键覆盖；
	//	③ 未配对   → 视为**新增**，按请求字段创建；
	//	④ 请求数组里没有的旧子记录 → 不进入新版本（即删除；数组即子记录集合）。
	//
	// 「字段未传」与「字段显式置空」由 key 存在性区分：key 不存在 → 保留旧值；
	// key 存在（含 null / ""）→ 覆盖，即显式清空。
	//
	// 生效范围：仅「OnUpdate + 请求携带子表 + 已有旧子记录」这条路径。**未携带子表**
	// 时仍走 DB 回填（本就带全字段），无需合并。
	//
	// 身份键只用于**匹配旧记录**，不改变各路径对主键的既有处理：版本化父的新快照
	// 仍是清 PK 重建（v3 通道填预分配值），非版本化父仍是删旧行 + 新建，旧版本子行
	// 不做任何修改。
	MergeChildrenOnUpdate bool

	// MergeChildrenOnVersionedRebuild **已弃用**：语义已从「版本化重建」扩展到
	// 「非版本化父 + 携带子表」，请改用 MergeChildrenOnUpdate（两者任一为 true 即生效，
	// 保留此字段仅为兼容早期采用者）。
	//
	// Deprecated: use MergeChildrenOnUpdate.
	MergeChildrenOnVersionedRebuild bool
