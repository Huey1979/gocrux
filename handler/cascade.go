...
type CascadeRelation struct {
	...
	// MergeChildrenOnVersionedRebuild 版本化父表**重建子快照**时，是否把请求携带的
	// 子记录与旧子记录按身份**逐字段合并**（而不是把请求当作完整快照直接重建）。
	//
	// 默认 false = 既有契约：请求携带的子数组即新版本的完整子内容，字段缺省 → 零值/默认值。
	// 置 true 后适用于「提交哪些字段就改哪些字段」的局部更新契约：
	//
	...
	//
	// 生效范围（重要）：仅「版本化父 + OnUpdate + 携带子表」这条**清 PK 重建**路径
	// （旧版本子行保留在旧父版本下，新版本复制重建）。两种情形不受影响：
	//   - 未携带子表 → 仍走 DB 回填（本就带全字段）；
	//   - 非版本化父 + 携带子表 → 仍是既有的「全量替换」语义（先删旧子行）。
	//
	// 身份键只用于**匹配旧记录**，不影响新快照的主键：新子记录一律清 PK 重建
