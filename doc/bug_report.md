...
| BUG-021 | 版本化更新级联子表主键冲突：passToChild=true 时未清除子数据旧 PK → 旧 PK 携带进 INSERT 导致 Duplicate entry | 本次 |
...
## BUG-021 详细分析：版本化更新级联子表主键冲突

### 问题

版本化表单更新时（`is_versioned=true`），级联子表 `sys_form_write_section` 的旧 `section_ulid` 未被清除，INSERT 新版本时触发 `Error 1062: Duplicate entry`。

**服务端日志**（`/data/go/heims/logs/business_2026-07-08.log`）：

```
INSERT INTO `sys_form_write_section` (...) VALUES ('01KWTJ32JMWDM808HY01TCSNYX', ...)
→ Error 1062 (23000): Duplicate entry '01KWTJ32JMWDM808HY01TCSNYX' for key 'PRIMARY'
```

### 根因

`handler/generic_write_impl.go:_doUpdate` 中级联子数据处理逻辑（第 237-247 行）：
...
旧 PK 清除逻辑包在 `!passParentVersioned` 条件内。当父表**版本化**时：
- `passParentVersioned=true` → `passToChild=true`（正确，子表需 CREATE）
- 但 `!passParentVersioned` 为 `false` → **旧 PK 未清除**
- 子表带旧 PK 走 INSERT → Duplicate entry

### 修复

将旧 PK 清除逻辑从 `!passParentVersioned` 条件中提出，改为检查 `passToChild && hasChildren`：
...
// 子记录的旧 PK 必须清除，否则 CREATE 时会与旧记录冲突
...
- 所有版本化实体的级联 Update 操作（`is_versioned=true` + `Cascades.OnUpdate=true`）
- 非版本化级联更新（BUG-018/BUG-019 场景）不受影响，行为不变
