...
| 编号 | 描述 | 严重性 | 建议修复 |
|------|------|--------|---------|
| BUG-024 | MongoCRUDRepository.List 完全忽略排序参数（OrderBy/OrderDir），MongoDB 查询无排序 | 🟡 中 | 见下方详细分析 |
| BUG-025 | _doGetByCode 硬编码 "is_deleted" 字段名，不兼容 MongoDB camelCase 实体 | 🟡 中 | 见下方详细分析 |
| BUG-026 | _doList 版本化分支未添加 isDeleted 过滤，软删除版本化记录会泄漏到列表 | 🟡 中 | 见下方详细分析 |
